...
public class Ring<T extends NodeItem> {
    ...
    private final Map<NodeModel<T>, Double> piePartBaseAngleMap = new HashMap<>();
    ...
    private final Map<NodeModel<T>, AtomicInteger> multiMap = new HashMap<>();
    private final AtomicInteger multi = new AtomicInteger(1);

    private Ring(TreeModel<T> model) {
        this.model = model;
    }
    ...
    public Map<NodeModel<T>, PointF> genPositions() {
        ...
        for (NodeModel<T> rootChild : rootNodeChildNodes) {
