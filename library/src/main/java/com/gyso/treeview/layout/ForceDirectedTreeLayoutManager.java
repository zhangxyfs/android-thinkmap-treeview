...
/**
 * 力导向树形布局管理器，继承自TreeLayoutManager并实现ForceListener接口
 * 用于实现基于力导向算法的树形结构布局
 *
 * @param <T> 节点项类型，必须继承自NodeItem
 */
public class ForceDirectedTreeLayoutManager<T extends NodeItem> extends TreeLayoutManager<T> implements ForceListener {
    private final static String TAG = ForceDirectedTreeLayoutManager.class.getSimpleName();
    private Force force;
    private final Handler handler;
    private TreeViewContainer<T> treeViewContainer;
    final Map<NodeModel<T>, FNode> fNodeCache = new HashMap<>();
    protected Map<NodeModel<T>, ViewBox> boxHashMap = null;
    ...
    private void setUpData() {
        ...
        if (mTreeModel != null) {
            ...
            fNodeCache.clear();
            mTreeModel.doTraversalNodes(new ITraversal<NodeModel<T>>() {
                @Override
                public void next(NodeModel<T> next) {
     ...
     * @return 视图半径
     */
    private float getViewRadius(@NonNull NodeModel<T> nodeMode) {
     ...
     * @param nodeMode 节点模型
     */
    private void setFNodePosition(FNode fNode, @NonNull NodeModel<T> nodeMode) {
        ViewBox viewBox = boxHashMap.get(nodeMode);
        if (viewBox == null) {
            NodeModel<T> parentNode = nodeMode.getParentNode();
            viewBox = new ViewBox();
    ...
    private void firstLayoutAfterMeasure(NodeModel<T> currentNode) {
    ...
    @Override
    public void performLayout(TreeViewContainer<T> treeViewContainer) {
        this.treeViewContainer = treeViewContainer;
        if (treeViewContainer != null) {
            final TreeModel<T> mTreeModel = treeViewContainer.getTreeModel();
            if (mTreeModel != null) {
     ...
     * @param treeViewContainer 树视图容器
     */
    private void layoutNodesByForceDirected(NodeModel<T> currentNode, TreeViewContainer<T> treeViewContainer) {
     ...
     * @param treeViewContainer 树视图容器
     */
    private void updateRootNodeDelta(TreeViewContainer<T> treeViewContainer) {
        NodeModel<T> rootNode = treeViewContainer.getTreeModel().getRootNode();
