...
/**
 * 紧凑型水平左右布局管理器，继承自CompactRightTreeLayoutManager
 * 该布局管理器将根节点的子节点分为两部分，一部分保持在右侧，另一部分镜像到左侧
 *
 * @param <T> 节点项类型，必须继承自NodeItem
 */
public class CompactHorizonLeftAndRightLayoutManager<T extends NodeItem> extends CompactRightTreeLayoutManager<T> {
    /**
     * 类的标签，用于调试日志
     */
