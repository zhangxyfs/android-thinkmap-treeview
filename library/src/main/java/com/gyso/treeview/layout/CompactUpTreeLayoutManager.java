...
/**
 * 紧凑型向上树形布局管理器，继承自CompactDownTreeLayoutManager
 * 该布局管理器将树形结构按照从下到上的垂直方向进行布局
 * 实现方式是先按向下布局计算位置，然后以垂直中心线为轴进行镜像翻转
 *
 * @param <T> 节点项类型，必须继承自NodeItem
 */
public class CompactUpTreeLayoutManager<T extends NodeItem> extends CompactDownTreeLayoutManager<T> {
    /**
     * 类的标签，用于调试日志
     */
